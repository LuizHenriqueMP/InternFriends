...
Com `MAIL_BACKEND=console`, os links de verificação e recuperação aparecem no terminal do Flask. Para usar o Resend:
...
O domínio do remetente precisa estar verificado no Resend. Nunca commite a chave da API; mantenha-a apenas no arquivo `.env` ou no ambiente de produção.
